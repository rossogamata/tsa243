...
> **Середовище:** Ubuntu Server 24.04 LTS / Proxmox VE / WSL2
...
### [Заняття 5.5 — Мережеві налаштування та передача файлів між системами](lesson5_5/)

- Команда `ip`: інтерфейси, маршрути, ARP
- Netplan: статична IP, DHCP, синтаксис YAML, `netplan try/apply`
- DNS: `/etc/hosts`, `resolvectl`, `dig`; діагностика: `ping`, `ss`, `tcpdump`
- OpenSSH: встановлення, підключення, аутентифікація за ключами, `~/.ssh/config`, `sshd_config`
- SCP: копіювання файлів і директорій між хостами
- Rsync: дельта-синхронізація, виключення, дзеркальний режим (`--delete`)
- SFTP: інтерактивна передача файлів

Середовище: Proxmox VE, мережа `192.168.177.0/24`. Включає [скрипт самоперевірки](lesson5_6/self_check/check.sh).

---

### [Заняття 5.6 — Мережева конфігурація (Практика)](lesson5_6/)

- Дослідження мережевих інтерфейсів: `ip a`, `ip route`, DNS
- Налаштування статичної IP через Netplan на VM1 (`192.168.177.101/24`)
- Налаштування `/etc/hosts` та hostname між двома VM
- Перевірка маршрутизації та доступу до зовнішньої мережі
- Діагностика: `ss`, `nc`, `tcpdump` з ICMP-фільтром
- Встановлення SSH, підключення між VM
- Сканування мережі: `nmap` (порти, версії, ping-scan підмережі)
- Аналіз журналу SSH: успішні та невдалі спроби входу

---

### [Заняття 5.7 — Утиліти віддаленого доступу до файлів (Практика)](lesson5_7/)

- Налаштування `sshd_config`: заборона root, вимкнення пароля, перевірка синтаксису
- Аутентифікація за ключами Ed25519: генерація, копіювання, права `~/.ssh/`
- Конфігурація SSH-клієнта `~/.ssh/config` з псевдонімами хостів
- SCP: передача файлів і директорій, збереження атрибутів, обмеження швидкості
- Rsync: перша синхронізація, інкрементальна передача, виключення файлів
- Rsync дзеркальний режим (`--delete`), обов'язковий `--dry-run` перед застосуванням
- Підсумкове завдання: скрипт `backup.sh` для автоматичного резервного копіювання
...
# ═══════════════════════════════════════════════════════════
# МЕРЕЖА (lesson5_5 / lesson5_6)
# ═══════════════════════════════════════════════════════════
ip a                                # інтерфейси з IP-адресами
ip a show eth0                      # конкретний інтерфейс
ip link set eth0 up/down            # увімкнути / вимкнути
ip route                            # таблиця маршрутизації
ip route get 8.8.8.8                # маршрут до конкретного хоста
sudo netplan try                    # застосувати з можливістю відкату
sudo netplan apply                  # застосувати без відкату
sudo sshd -t                        # перевірити синтаксис sshd_config
resolvectl status                   # DNS-сервери по інтерфейсах
dig +short google.com               # DNS-запит
ping -c 4 HOST                      # ICMP-пінг (4 пакети)
traceroute HOST                     # маршрут до хоста
ss -tlnp                            # TCP LISTEN + процес
ss -tunap                           # всі активні з'єднання
nc -zv HOST PORT                    # перевірити доступність порту
sudo tcpdump -i eth0 icmp           # захопити ICMP-пакети
nmap HOST                           # сканування портів
nmap -sV HOST                       # версії сервісів
nmap -sn 192.168.177.0/24          # виявлення хостів у мережі

# ═══════════════════════════════════════════════════════════
# SSH / SCP / RSYNC (lesson5_5 / lesson5_7)
# ═══════════════════════════════════════════════════════════
ssh user@HOST                       # підключення
ssh -i ~/.ssh/id_ed25519 user@HOST  # з вказаним ключем
ssh user@HOST "команда"             # виконати команду без сесії
ssh-keygen -t ed25519 -C "коментар" # генерація ключа Ed25519
ssh-copy-id -i ~/.ssh/id_ed25519.pub user@HOST  # копіювання ключа
sudo systemctl reload ssh           # перечитати sshd_config
journalctl -u ssh -f                # журнал SSH у реальному часі
scp file user@HOST:/path/           # копіювати файл на сервер
scp -r dir/ user@HOST:/path/        # рекурсивно (директорія)
scp -p file user@HOST:/path/        # зі збереженням атрибутів
rsync -avzP src/ user@HOST:/dst/    # синхронізація з прогресом
rsync -avzn src/ user@HOST:/dst/    # симуляція (dry-run)
rsync -avz --delete src/ user@HOST:/dst/  # дзеркало
