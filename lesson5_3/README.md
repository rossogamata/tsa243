...
### 1.11 Текстові прогрес-бари та гістограми

Текстова візуалізація у bash будується через `printf` та арифметику. Ключова ідея — обчислити ширину смуги як частку від максимуму та вивести потрібну кількість символів у циклі.

#### Прогрес-бар

```bash
#!/bin/bash
# Функція: print_bar ВІДСОТОК ШИРИНА
# Виводить рядок типу: [████████░░░░░░░░░░░░]  42%

print_bar() {
    local PERCENT="$1"   # 0..100
    local WIDTH="$2"     # ширина смуги у символах, наприклад 40

    local FILLED=$(( PERCENT * WIDTH / 100 ))
    local EMPTY=$(( WIDTH - FILLED ))

    # Сформувати рядок заповнених символів
    local BAR_FILLED=""
    for ((i=0; i<FILLED; i++)); do BAR_FILLED+="█"; done

    # Сформувати рядок порожніх символів
    local BAR_EMPTY=""
...
> **Головна формула:** `FILLED = PERCENT * WIDTH / 100`
> Bash виконує лише цілочисельну арифметику, тому дрібна частина відкидається — це нормально для відображення.

#### Горизонтальна гістограма

Гістограма відрізняється від прогрес-бару тим, що масштаб смуги визначається **відносно максимального значення у наборі**, а не відносно 100%.

```bash
#!/bin/bash
# Масив значень: кількість рядків у лог-файлі по годинах (0..23)
declare -A COUNTS
for HOUR in {0..23}; do COUNTS[$HOUR]=0; done

# Заповнити тестовими даними
COUNTS[8]=150; COUNTS[9]=320; COUNTS[10]=280; COUNTS[11]=190
COUNTS[12]=95; COUNTS[13]=410; COUNTS[14]=375; COUNTS[15]=220

# Знайти максимальне значення (для масштабування)
MAX=0
for HOUR in "${!COUNTS[@]}"; do
    [ "${COUNTS[$HOUR]}" -gt "$MAX" ] && MAX="${COUNTS[$HOUR]}"
done

# Ширина смуги у символах
BAR_WIDTH=40
...
for HOUR in $(seq -w 0 23); do
    ...
    # Ширина смуги пропорційна до максимуму
    if [ "$MAX" -gt 0 ]; then
...
#### Підрахунок подій за годиною з лог-файлу

Типова задача перед побудовою гістограми — витягти годину з кожного рядка логу:

```bash
# Формат syslog: "Mar 13 09:42:15 ..."
# Поле $3 = "09:42:15", перші два символи = година

declare -A COUNTS
for HOUR in {0..23}; do COUNTS[$HOUR]=0; done

while IFS= read -r LINE; do
    # Витягти годину: взяти третє поле, відрізати до першого двокрапки
    HOUR=$(echo "$LINE" | awk '{print $3}' | cut -d: -f1)
    # Перевірити, що це число
    if [[ "$HOUR" =~ ^[0-9]{2}$ ]]; then
        (( COUNTS[10#$HOUR]++ ))   # 10# — примусово десяткова система
    fi
done < /var/log/syslog

# Далі — будуємо гістограму як показано вище
```

> **Увага на `10#`:** у bash числа з нулем на початку (`08`, `09`) інтерпретуються як вісімкові. Префікс `10#` примусово вказує десяткову систему числення.

#### Альтернатива: `printf` для повторення символу

```bash
# Замість циклу for — printf з форматом
BAR=$(printf '%0.s█' $(seq 1 $FILLED))$(printf '%0.s░' $(seq 1 $EMPTY))

# Або через parameter expansion (bash 3+)
FILL_STR=$(printf '%.0s█' $(seq 1 40))     # рядок з 40 символів █
BAR="${FILL_STR:0:$FILLED}"                # взяти перші FILLED символів
```
